...
<nav aria-label="Page navigation" class="px-1">
    <ul class="pagination set-flex gap-[3px]">
        ...
        <li class="rounded-md hover:bg-light-shadow dark:hover:bg-dark-shadow set-flex <?= $isFirstPage ? 'cursor-not-allowed' : '' ?>">
            <?php if (!$isFirstPage) : ?>
                <a href="<?= generatedPagePaginationUrl('pelanggaran', $_GET, 'page') . session()->get('previousPagePelanggaran') ?>"
                    aria-label="<?= lang('Pager.previous') ?>" class="p-2">
                <?php else : ?>
                    <span class="p-2">
                    <?php endif; ?>
        ...
        <?php foreach ($pager->links() as $link): ?>
            <li <?= $link['active'] ? 'class="active"' : '' ?>>
                <a href="<?= generatedPagePaginationUrl('pelanggaran', $_GET, 'page') . $link['title']; ?>" class="rounded-md py-1.5 px-2.5 hover:bg-light-shadow dark:hover:bg-dark-shadow">
                    <?= $link['title'] ?>
                </a>
            </li>
        <?php endforeach ?>
        ...
        <li class="rounded-md hover:bg-light-shadow dark:hover:bg-dark-shadow set-flex <?= $isLastPage ? 'cursor-not-allowed' : '' ?>">
            <?php if (!$isLastPage) : ?>
                <a href="<?= generatedPagePaginationUrl('pelanggaran', $_GET, 'page') . session()->get('nextPagePelanggaran') ?>"
