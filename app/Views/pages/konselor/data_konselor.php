...
<div class="w-full">
    ...
    <ul class="flex items-center gap-2">
        ...
        <li class="text-light-text dark:text-dark-text text-xs md:text-sm">/</li>
        <li>
            <a href="<?= site_url('/konselor'); ?>" class="text-danger m-0 text-xs md:text-sm">Data Konselor</a>
        </li>
    </ul>

    <!-- Header Section -->
    <header class="mb-3 mt-5 flex justify-between flex-row gap-2 md:gap-0">
        <h1 class="text-xl md:text-2xl font-bold pt-1 text-light-text dark:text-dark-text">Data Konselor (<span><?= $totalKonselor; ?></span>)</h1>
        ...
        <div class="modal opacity-0 pointer-events-none transition-all duration-200 fixed inset-0 z-50 bg-dark-bg/60 dark:bg-light-bg/10 set-flex items-start">
            <div class="modal-content scale-0 transition-all duration-200 shadow-md w-full lg:w-3/5 overflow-auto m-0 max-h-screen mt-2">
                <?= $this->include('pages/konselor/tambah_konselor'); ?>
            </div>
        </div>
    </header>
            ...
            <ul class="set-flex items-start md:items-center bg-light-card dark:bg-dark-card p-4 gap-3 rounded-xl shadow-md mb-6">
                <li class="w-full">
                    ...
                    <div class="box-search-input">
                        ...
                        <input type="search" name="key" id="key" class="search-input" placeholder="Cari untuk nama konselor dan total konseling" value="<?= $keyword ?? ''; ?>">
                    </div>
    ...
    <section class="flex flex-col bg-light-card dark:bg-dark-card shadow-md rounded-xl">
        <div class="overflow-x-auto bg-light-card dark:bg-dark-card shadow-md rounded-xl overflow-hidden">
            <table class="w-full bg-light-card dark:bg-dark-card text-light-text dark:text-dark-text border border-none">
                    ...
                    <tr>
                        ...
                        <th class="border border-light-shadow dark:border-dark-shadow px-4 py-3">
                            <div class="set-flex gap-2">
                                <span>Nama Konselor</span>
                                <a href="<?= site_url('/konselor' . ($keyword ? '?key=' . $keyword . '&' : '?') . 'sort=nama_konselor&order=' . ($next_order ?? 'desc')); ?>">
                        ...
                        <th class="border border-light-shadow dark:border-dark-shadow px-4 py-3">No Telp</th>
                        <th class="border border-light-shadow dark:border-dark-shadow px-4 py-3">
                            <div class="set-flex gap-2">
                                <span>Total Konseling</span>
                                <a href="<?= site_url('/konselor' . ($keyword ? '?key=' . $keyword . '&' : '?') . 'sort=total_konseling&order=' . ($next_order ?? 'desc')); ?>">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor" class="w-2.5 cursor-pointer text-light-text dark:text-dark-text">
                                        <path d="M137.4 41.4c12.5-12.5 32.8-12.5 45.3 0l128 128c9.2 9.2 11.9 22.9 6.9 34.9s-16.6 19.8-29.6 19.8L32 224c-12.9 0-24.6-7.8-29.6-19.8s-2.2-25.7 6.9-34.9l128-128zm0 429.3l-128-128c-9.2-9.2-11.9-22.9-6.9-34.9s16.6-19.8 29.6-19.8l256 0c12.9 0 24.6 7.8 29.6 19.8s2.2 25.7-6.9 34.9l-128 128c-12.5 12.5-32.8 12.5-45.3 0z" />
                                    </svg>
                                </a>
                ...
                <tbody>
                    <?php $no = 1; ?>
                    <?php if (isset($konselor) && !empty($konselor)) : ?>
                        <?php foreach ($konselor as $kslr) : ?>
                            <tr class="odd-even-row">
                                <th class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= $no++; ?></th>

                                <?php if (esc($kslr['foto'] === "-") || esc($kslr['foto'] === null)) : ?>
                                    <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($kslr['foto']); ?></td>
                                <?php else : ?>
                                    <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2 relative"><img src="<?= base_url('./uploads/img_konselor/' . esc($kslr['foto'])); ?>" alt="foto_siswa" class="size-10 rounded-[50%] absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2"></td>
                                <?php endif ?>

                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($kslr['nama_konselor']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($kslr['no_telp']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-3 py-2"><?= esc($kslr['total_konseling']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2.5">
                                    <div class="set-flex gap-1.5">
                                        <a href="<?= $urls[$kslr['id_konselor']]; ?>" class="btn-basic mb-0 px-4 py-1">Edit</a>
                                        <form action="<?= site_url('/konselor/delete/' . esc($kslr['id_konselor']) . '?' . http_build_query($_GET )); ?>" method="post">
                    ...
                    <?php else : ?>
                        <tr class="text-center">
                            <td class="border border-light-shadow dark:border-dark-shadow px-4 py-5 text-xl" colspan="5">Data Konselor belum ada.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div class="modal opacity-0 pointer-events-none transition-all duration-200 fixed inset-0 z-50 bg-dark-bg/60 dark:bg-light-bg/10 set-flex items-start">
    <div class="modal-content scale-0 transition-all duration-200 shadow-md w-full lg:w-3/5 overflow-auto m-0 max-h-screen mt-2">
        <?php if (isset($kslr)) : ?>
            <?= $this->include('pages/konselor/edit_konselor'); ?>
        <?php endif; ?>
    </div>
</div>
