...
<div class="w-full">
    ...
    <ul class="flex items-center gap-2">
        ...
        <li class="text-light-text dark:text-dark-text text-xs md:text-sm">/</li>
        <li>
            <a href="<?= site_url('/guru'); ?>" class="text-danger m-0 text-xs md:text-sm">Data Guru</a>
        </li>
    </ul>

    <!-- Header Section -->
    <header class="mb-3 mt-5 flex justify-between flex-row gap-2 md:gap-0">
        <h1 class="text-xl md:text-2xl font-bold pt-1 text-light-text dark:text-dark-text">Data Guru (<span><?= $totalGuru; ?></span>)</h1>
        ...
        <div class="modal opacity-0 pointer-events-none transition-all duration-200 fixed inset-0 z-50 bg-dark-bg/60 dark:bg-light-bg/10 set-flex items-start">
            <div class="modal-content scale-0 transition-all duration-200 shadow-md w-full lg:w-3/5 overflow-auto m-0 max-h-screen">
                <?= $this->include('pages/guru/tambah_guru'); ?>
            </div>
        </div>
    </header>
            ...
            <ul class="set-flex flex-col items-start md:items-center md:flex-row  bg-light-card dark:bg-dark-card p-4 gap-3 rounded-xl shadow-md mb-6">
                <li class="w-full">
                    ...
                    <div class="box-search-input">
                        ...
                        <input type="search" name="key" id="key" class="search-input" placeholder="Cari untuk nama guru, wali kelas apa dan lain - lain" value="<?= $keyword ?? ''; ?>">
                    </div>

                </li>
                <li class="w-full md:w-1/2">
                    <label for="kategori_mapel" class="block mb-1 text-light-text dark:text-dark-text">Kategori Mata Pelajaran</label>
                    <select name="kategori_mapel" id="kategori_mapel" class="category-input">
                        <option value="" selected>Semua</option>
                        <?php if (isset($mapel)) : ?>
                            <?php foreach ($mapel as $row) : ?>
                                <?php $valueMapel = strtolower(esc($row['mata_pelajaran'])); ?>

                                <option value="<?= $valueMapel; ?>"
                                    <?= (strtolower($kategori_mapel ?? '') == $valueMapel) ? 'selected' : '' ?>>
                                    <?= esc($row['mata_pelajaran']); ?>
                                </option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
    ...
    <section class="flex flex-col bg-light-card dark:bg-dark-card shadow-md rounded-xl">
        <ul class="flex items-start justify-between flex-col lg:flex-row gap-4 lg:gap-0 w-full p-4 text-light-text dark:text-dark-text">
            <li class="w-full flex items-start lg:items-center flex-col md:flex-row lg:flex-row gap-3 lg:gap-0 m-0 lg:ml-1">
                <div class="w-full md:w-1/2 md:set-flex md:flex-col md:gap-y-1 md:items-start lg:w-auto lg:block">
                    ...
                    <select name="show" id="show" class="show-select category-input w-full lg:w-36 m-0 lg:mr-[0.8rem] lg:ml-2">
                        ...
                        <?php foreach ($options as $option) : ?>
                            <?php if ($totalGuru >= $option) : ?>
                                <option value="<?= $option; ?>" <?= is_selected($show, $option, $totalGuru); ?>><?= $option; ?></option>
                            <?php endif; ?>
                        <?php endforeach; ?>
                        <option value="<?= $totalGuru; ?>" <?= is_selected($show, $totalGuru, $totalGuru); ?>>Semua Baris</option>
                    </select>
                </div>
                ...
                <div class="w-full md:w-1/2 md:set-flex md:flex-col md:gap-y-1 md:items-start lg:w-auto lg:block">
                    ...
                    <select name="sort" id="sort" class="sort-select category-input w-full lg:w-40 m-0 lg:mr-[0.8rem] lg:ml-2">
                        <option value="nama_guru" <?= is_selected($sort, 'nama_guru'); ?>>Nama Guru</option>
                        <option value="mata_pelajaran" <?= is_selected($sort, 'mata_pelajaran'); ?>>Mata Pelajaran</option>
                        <option value="wali_kelas" <?= is_selected($sort, 'wali_kelas'); ?>>Wali Kelas</option>
            ...
            <li class="self-center lg:self-auto">
                <?= $pager->links('default', 'guru_pagination'); ?>
            </li>
        </ul>
            ...
            <table class="w-full bg-light-card dark:bg-dark-card text-light-text dark:text-dark-text border border-none">
                <thead>
                    <tr>
                        ...
                        <th class="border border-light-shadow dark:border-dark-shadow px-4 py-3">Nama Guru</th>
                        <th class="border border-light-shadow dark:border-dark-shadow px-4 py-3">Mata Pelajaran</th>
                        <th class="border border-light-shadow dark:border-dark-shadow px-4 py-3">Wali Kelas</th>
                ...
                <tbody>
                    ...
                    <?php if (isset($guru) && !empty($guru)) : ?>
                        <?php foreach ($guru as $gru) : ?>
                            <tr class="odd-even-row">
                                ...
                                <?php if (esc($gru['foto'] === "-") || esc($gru['foto'] === null)) : ?>
                                    <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($gru['foto']); ?></td>
                                <?php else : ?>
                                    <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2 relative"><img src="<?= base_url('./uploads/img_guru/' . esc($gru['foto'])); ?>" alt="foto_siswa" class="size-10 rounded-[50%] absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2"></td>
                                <?php endif ?>

                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($gru['nama_guru']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($gru['mata_pelajaran']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($gru['wali_kelas']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2.5">
                                    <div class="set-flex gap-1.5">
                                        <a href="<?= $urlsToEditGuru[$gru['id_guru']]; ?>" class="btn-basic mb-0 px-4 py-1">Edit</a>
                                        <form action="<?= site_url('/guru/delete/' . esc($gru['id_guru']) . '?' . http_build_query($_GET)); ?>" method="post">
                    ...
                    <?php else : ?>
                        <tr class="text-center">
                            <td class="border border-light-shadow dark:border-dark-shadow px-4 py-5 text-xl" colspan="6">Data Guru belum ada.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div class="modal opacity-0 pointer-events-none transition-all duration-200 fixed inset-0 z-50 bg-dark-bg/60 dark:bg-light-bg/10 set-flex items-start">
    <div class="modal-content scale-0 transition-all duration-200 shadow-md w-full lg:w-3/5 overflow-auto m-0 max-h-screen">
        <?php if (isset($gru)) : ?>
            <?= $this->include('pages/guru/edit_guru'); ?>
        <?php endif; ?>
    </div>
</div>
