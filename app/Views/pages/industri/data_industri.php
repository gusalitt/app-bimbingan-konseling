...
<div class="w-full">
    ...
    <ul class="flex items-center gap-2">
        ...
        <li>
            <a href="<?= site_url('/industri'); ?>" class="text-danger m-0 text-xs md:text-sm">Data Industri</a>
        </li>
    </ul>

    <!-- Header Section -->
    <header class="mb-3 mt-5 flex justify-between flex-row gap-2 md:gap-0">
        <h1 class="text-xl md:text-2xl font-bold pt-1 text-light-text dark:text-dark-text">Data Industri (<span><?= $totalIndustri; ?></span>)</h1>
        ...
        <div class="modal opacity-0 pointer-events-none transition-all duration-200 fixed inset-0 z-50 bg-dark-bg/60 dark:bg-light-bg/10 set-flex items-start">
            <div class="modal-content scale-0 transition-all duration-200 shadow-md w-full lg:w-3/5 overflow-auto m-0 max-h-screen">
                <?= $this->include('pages/industri/tambah_industri'); ?>
            </div>
        </div>
    </header>
            ...
            <ul class="set-flex flex-col items-start md:items-center md:flex-row  bg-light-card dark:bg-dark-card p-4 gap-3 rounded-xl shadow-md mb-6">
                <li class="w-full">
                    ...
                    <div class="box-search-input">
                        ...
                        <input type="search" name="key" id="key" class="search-input" placeholder="Cari untuk nama siswa, tempat industri dan lain - lain" value="<?= $keyword ?? ''; ?>">
                    </div>
                ...
                <li class="w-full md:w-1/2">
                    ...
                    <select name="kategoriStatus" id="kategoriStatus" class="category-input">
                        ...
                        <option value="aktif" <?= is_selected($kategori_status, 'aktif'); ?>>Aktif</option>
                        <option value="non-aktif" <?= is_selected($kategori_status, 'non-aktif'); ?>>Non-Aktif</option>
                    </select>
                </li>
    ...
    <section class="flex flex-col bg-light-card dark:bg-dark-card shadow-md rounded-xl">
        <ul class="flex items-start justify-between flex-col lg:flex-row gap-4 lg:gap-0 w-full p-4 text-light-text dark:text-dark-text">
                ...
                <div class="w-full md:w-1/2 md:set-flex md:flex-col md:gap-y-1 md:items-start lg:w-auto lg:block">
                    ...
                    <select name="show" id="show" class="show-select category-input w-full lg:w-36 m-0 lg:mr-[0.8rem] lg:ml-2">
                        ...
                        <?php foreach ($options as $option) : ?>
                            <?php if ($totalIndustri >= $option) : ?>
                                <option value="<?= $option; ?>" <?= is_selected($show, $option, $totalIndustri); ?>><?= $option; ?></option>
                            <?php endif; ?>
                        <?php endforeach; ?>
                        <option value="<?= $totalIndustri; ?>" <?= is_selected($show, $totalIndustri, $totalIndustri); ?>>Semua Baris</option>
                    </select>
                </div>

                <div class="w-full md:w-1/2 md:set-flex md:flex-col md:gap-y-1 md:items-start lg:w-auto lg:block">
                    <label for="sort">Urutkan : </label>
                    <select name="sort" id="sort" class="sort-select category-input w-full lg:w-44 m-0 lg:mr-[0.8rem] lg:ml-2">
                        <option value="nama_siswa" <?= is_selected($sort, 'nama_siswa'); ?>>Nama siswa</option>
                        <option value="tempat_industri" <?= is_selected($sort, 'tempat_industri'); ?>>Tempat Industri</option>
                        <option value="tgl_mulai" <?= is_selected($sort, 'tgl_mulai'); ?>>Tanggal Mulai</option>
                        <option value="tgl_selesai" <?= is_selected($sort, 'tgl_selesai'); ?>>Tanggal Selesai</option>
            ...
            <li class="self-center lg:self-auto">
                <?= $pager->links('default', 'industri_pagination'); ?>
            </li>
        </ul>
            ...
            <table class="w-full bg-light-card dark:bg-dark-card text-light-text dark:text-dark-text border border-none">
                <thead>
                    <tr>
                        ...
                        <th class="border border-light-shadow dark:border-dark-shadow px-3 py-3">Tempat</th>
                        <th class="border border-light-shadow dark:border-dark-shadow px-1 py-3">Tgl Mulai</th>
                        <th class="border border-light-shadow dark:border-dark-shadow px-1 py-3">Tgl Selesai</th>
                        <th class="border border-light-shadow dark:border-dark-shadow px-3 py-3">Status</th>
                        <th class="border border-light-shadow dark:border-dark-shadow px-3 py-3">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    <?php $no = $itemStart + 1; ?>
                    <?php if (isset($industri) && !empty($industri)) : ?>
                        <?php foreach ($industri as $indstr) : ?>
                            <tr class="odd-even-row">
                                <th class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= $no++; ?></th>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($indstr['nama_siswa']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($indstr['kelass']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= esc($indstr['tempat_industri']); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= date("d-m-Y", strtotime(esc($indstr['tgl_mulai']))); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2"><?= date("d-m-Y", strtotime(esc($indstr['tgl_selesai']))); ?></td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2">

                                    <?php
                                    $bg = match (esc($indstr['status'])) {
                                        'aktif' => 'bg-green-600',
                                        'non-aktif'  => 'bg-red-600',
                                        default  => 'bg-gray-600',
                                    };
                                    ?>
                                    <span class="<?= $bg; ?> rounded-3xl py-1 px-3 w-max inline-block text-center text-sm text-white"><?= ucwords(esc($indstr['status'])); ?></span>

                                </td>
                                <td class="border border-light-shadow dark:border-dark-shadow px-4 py-2.5">
                                    <div class="set-flex gap-1.5">
                                        <a href="<?= $urlsToEditIndustri[$indstr['id_industri']]; ?>" class="btn-basic mb-0 px-4 py-1">Edit</a>
                                        <form action="<?= site_url('/industri/delete/' . esc($indstr['id_industri']) . '?' . http_build_query($_GET)); ?>" method="post">
                    ...
                    <?php else : ?>
                        <tr class="text-center">
                            <td class="border border-light-shadow dark:border-dark-shadow px-4 py-5 text-xl" colspan="8">Data Industri belum ada.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div class="modal opacity-0 pointer-events-none transition-all duration-200 fixed inset-0 z-50 bg-dark-bg/60 dark:bg-light-bg/10 set-flex items-start">
    <div class="modal-content scale-0 transition-all duration-200 shadow-md w-full lg:w-3/5 overflow-auto m-0 max-h-screen">
        <?php if (isset($indstr)) : ?>
            <?= $this->include('pages/industri/edit_industri'); ?>
        <?php endif; ?>
    </div>
</div>
